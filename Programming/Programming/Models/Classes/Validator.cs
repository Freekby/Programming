namespace Programming
{
    static class Validator
    {
        static public void AssertOnPositiveValue(int value, string propertyName)
        {
            if (value < 0) 
            {  
                throw new ArgumentOutOfRangeException($"Поле {propertyName} должно быть больше 0"); 
            }
        }
        ...
        static public void AssertValueInRange(int value1, int min, int max, string propertyName)
        {
            ...
            {
                throw new ArgumentOutOfRangeException($"Поле {propertyName} должно находиться в диапазоне от {min} до {max}");
            }
        }
